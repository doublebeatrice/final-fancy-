...

{
  ...
  assert.strictEqual(report.schedule.startTime, '08:00');
  ...
  assert.strictEqual(report.commands.windowsTaskAction.execute, 'powershell.exe');
  assert.ok(report.commands.windowsTaskAction.arguments.includes("$env:AGENT_TODAY=(Get-Date).ToString('yyyy-MM-dd')"));
  assert.ok(report.commands.windowsTaskAction.arguments.includes('run_agent_unattended_supervisor.js'));
  assert.ok(!report.commands.windowsTaskAction.arguments.includes('--today 2026-05-25'));
  assert.ok(report.commands.windowsTaskAction.arguments.includes("'--command-timeout-ms'"));
  assert.ok(report.commands.windowsTaskAction.arguments.includes("'30000'"));
  assert.ok(report.commands.windowsTaskAction.arguments.includes('unattended_supervisor_task.log'));
  assert.ok(report.commands.windowsCompletionAuditAction.arguments.includes("$env:AGENT_TODAY=(Get-Date).ToString('yyyy-MM-dd')"));
  ...
  assert.ok(report.commands.windowsCompletionAuditAction.arguments.includes("'--natural-schedule-tolerance-minutes'"));
  assert.ok(report.commands.windowsCompletionAuditAction.arguments.includes("'15'"));
  ...
  assert.ok(report.commands.windowsCompletionAuditAction.arguments.includes("'--scheduled-task-name'"));
  assert.ok(report.commands.windowsCompletionAuditAction.arguments.includes("'AdOpsAgentCompletionAudit'"));
  assert.strictEqual(report.schedule.completionAudit.taskName, 'AdOpsAgentCompletionAudit');
  assert.strictEqual(report.schedule.completionAudit.startTime, '08:20');
