...
const codexApproval = {
  ...
  approvedBy: 'codex',
  actionSource: ['codex'],
  goal: { metric: 'orders', from: 1, to: 2, deadlineDays: 7, hardFloor: 0 },
  killSwitch: { metric: 'orders', condition: 'spend rises without orders by day 7', rollbackIf: 'spend rises without orders by day 7' },
};
...

{
  ...
  assert.deepStrictEqual(validated.plan[0].actions[0].reviewPlan.checkAfterDays, [3, 7]);
  assert.strictEqual(validated.plan[0].actions[0].reviewPlan.rollbackIf, 'spend rises without orders');
  assert.deepStrictEqual(validated.plan[0].actions[0].goal, {
    metric: 'orders',
    from: 1,
    to: 2,
    deadlineDays: 7,
  ...
  assert.strictEqual(validated.plan[0].actions[0].killSwitch.metric, 'orders');
}

{
      ...
      actions: [
        {
          ...
          currentBid: 0.5,
          suggestedBid: 0.55,
          reason: 'goal cannot prove anything',
          ...codexApproval,
          goal: { metric: 'orders', from: 1, to: 1, deadlineDays: 7, hardFloor: 0 },
          evidence: ['7d orders=1'],
          confidence: 0.8,
          riskLevel: 'low',
        },
  ...
  assert.ok(validated.review[0].action.reason.includes('risk_gate:missing_action_goal:from_equals_to'));
}
...

{
  ...
  const validated = validateAndNormalizePlan([
    {
      sku: 'SKU-1',
      summary: 'rule generator candidate must wait for AI approval',
      actions: [
        {
          entityType: 'keyword',
          id: 'kw-1',
          ...
          candidateActionType: 'bid',
          candidateReason: 'over_budget, bad_conversion',
          requiresAiDecision: true,
          approvedBy: null,
          currentBid: 0.5,
          suggestedBid: 0.55,
          reason: 'candidate generated by rule script',
          goal: { metric: 'orders', from: 1, to: 2, deadlineDays: 7, hardFloor: 0 },
          killSwitch: { metric: 'orders', condition: 'spend rises without orders by day 7' },
  ...
  assert.strictEqual(validated.review.length, 1);
  assert.strictEqual(validated.plan[0].actions.length, 0);
  assert.strictEqual(validated.review[0].action.decisionStage, 'candidate');
...

{
  ...
  const validated = validateAndNormalizePlan([
    {
      sku: 'SKU-1',
      summary: 'rule-generator candidate cannot execute even with partial approval',
      actions: [
        {
          entityType: 'keyword',
          id: 'kw-1',
          ...
          candidateSource: 'rule_generator',
          candidateActionType: 'bid',
          candidateReason: 'approved after review',
          requiresAiDecision: true,
          approvedBy: 'codex',
          currentBid: 0.5,
          suggestedBid: 0.55,
          reason: 'approved bid raise',
          goal: { metric: 'orders', from: 1, to: 2, deadlineDays: 7, hardFloor: 0 },
          killSwitch: { metric: 'orders', condition: 'spend rises without orders by day 7' },
          evidence: ['ai reviewed generator evidence'],
          confidence: 0.8,
          riskLevel: 'low',
          actionSource: ['codex'],
        },
...

{
  ...
  const validated = validateAndNormalizePlan([
    {
      sku: 'SKU-1',
      summary: 'missing source must not default to codex',
      actions: [
        {
          entityType: 'keyword',
          id: 'kw-1',
          ...
          currentBid: 0.5,
          suggestedBid: 0.55,
          reason: 'external schema without approval source',
          goal: { metric: 'orders', from: 1, to: 2, deadlineDays: 7, hardFloor: 0 },
          killSwitch: { metric: 'orders', condition: 'spend rises without orders by day 7' },
          evidence: ['missing actionSource'],
          confidence: 0.8,
          riskLevel: 'low',
        },
      ],
...
{
  const context = { products: buildProductContexts(cards, rowsByType, [], [], []).products };
  const baseApprovedAction = {
    entityType: 'keyword',
    id: 'kw-1',
    actionType: 'bid',
    currentBid: 0.5,
    suggestedBid: 0.55,
    reason: 'approved boundary test',
    evidence: ['boundary test'],
    confidence: 0.8,
    riskLevel: 'low',
    ...codexApproval,
  };
  const runOne = action => validateAndNormalizePlan([
    { sku: 'SKU-1', summary: 'approval gate case', actions: [action] },
  ], context);

  const withoutApprovedBy = { ...baseApprovedAction };
  delete withoutApprovedBy.approvedBy;
  ...
  const missingDecisionStage = runOne(withoutDecisionStage);
  assert.strictEqual(missingDecisionStage.review.length, 1);
  assert.strictEqual(missingDecisionStage.plan[0].actions.length, 0);
  assert.ok(missingDecisionStage.review[0].action.reason.includes('decisionStage_not_approved'));

  const generatorSource = runOne({
    ...baseApprovedAction,
    actionSource: ['generator_candidate', 'rule_generator'],
  });
  ...
  assert.strictEqual(codexSourceButCandidateStage.review.length, 1);
  assert.strictEqual(codexSourceButCandidateStage.plan[0].actions.length, 0);
  assert.ok(codexSourceButCandidateStage.review[0].action.reason.includes('decisionStage_not_approved'));

  const provisionalSource = runOne({
    ...baseApprovedAction,
    source: 'provisional_local_policy',
    boardExecutableHint: true,
  });
  assert.strictEqual(provisionalSource.review.length, 1);
  assert.strictEqual(provisionalSource.plan[0].actions.length, 0);
  assert.ok(provisionalSource.review[0].action.reason.includes('source_provisional_local_policy'));

  const approvedCodex = runOne(baseApprovedAction);
  assert.strictEqual(approvedCodex.errors.length, 0);
  assert.strictEqual(approvedCodex.review.length, 0);
...
{
  const files = [
    'scripts/generators/generate_over_budget_bad_conversion_schema.js',
    'scripts/generators/generate_over_budget_schema.js',
    'scripts/generators/generate_profit_create_schema.js',
    'scripts/generators/generate_all_sku_ops_schema.js',
    'scripts/generators/generate_ful_full_loop_schema.js',
    'scripts/generators/generate_weekly_focus_schema.js',
    'scripts/execute/audit_created_campaign_keywords.js',
  ];
  for (const file of files) {
    const content = fs.readFileSync(path.join(__dirname, '..', file), 'utf8');
    assert.strictEqual(content.includes("actionSource: ['codex']"), false, `${file} must not emit codex source from generator logic`);
    assert.ok(content.includes("decisionStage: 'candidate'"), `${file} must mark generated output as candidate`);
    assert.ok(content.includes("candidateSource: 'rule_generator'"), `${file} must mark rule generator source`);
    assert.ok(content.includes("requiresAiDecision: true"), `${file} must require AI or manual approval`);
  }
}
...

{
  ...
  const validated = validateAndNormalizePlan([
    {
      sku: 'SKU-1',
      summary: 'sb campaign budget repair',
      actions: [
        {
          entityType: 'sbCampaign',
          id: 'sbc1',
          ...
          currentBudget: 5,
          suggestedBudget: 6,
          reason: 'recover capped SB campaign',
          evidence: ['single sku SB campaign with orders'],
          confidence: 0.8,
...
{
  const highVolumeCards = [{ ...cards[0], unitsSold_30d: 100 }];
  const context = { products: buildProductContexts(highVolumeCards, rowsByType, [], [], []).products };
  const smallAbsoluteMove = validateAndNormalizePlan([
    {
      sku: 'SKU-1',
      summary: 'small absolute bid step on low CPC',
      actions: [
        {
          entityType: 'keyword',
          id: 'kw-1',
          ...
          currentBid: 0.12,
          suggestedBid: 0.14,
          reason: 'low CPC winner needs a small step up',
          evidence: ['small absolute bid delta'],
          confidence: 0.82,
          riskLevel: 'high_efficiency_controlled_bid_up',
          ...codexApproval,
        },
      ],
    },
  ], context);
  assert.strictEqual(smallAbsoluteMove.errors.length, 0);
  assert.strictEqual(smallAbsoluteMove.review.length, 0);
  assert.strictEqual(smallAbsoluteMove.plan[0].actions.length, 1);

  const largeAbsoluteMove = validateAndNormalizePlan([
    {
      sku: 'SKU-1',
      summary: 'large absolute bid step on high volume SKU',
      actions: [
        {
          entityType: 'keyword',
          id: 'kw-1',
          ...
          currentBid: 0.3,
          suggestedBid: 0.36,
          reason: 'larger CPC step needs review',
          evidence: ['large absolute bid delta'],
          confidence: 0.82,
          riskLevel: 'high_efficiency_controlled_bid_up',
          ...codexApproval,
        },
  ...
  assert.strictEqual(largeAbsoluteMove.errors.length, 0);
  assert.strictEqual(largeAbsoluteMove.review.length, 1);
  assert.ok(largeAbsoluteMove.review[0].action.reason.includes('high_volume_strong_bid_change'));
  assert.ok(largeAbsoluteMove.review[0].action.reason.includes('delta=0.06'));
}
