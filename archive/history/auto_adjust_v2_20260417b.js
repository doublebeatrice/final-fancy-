...
async function run() {
  ...
  // 7. 未覆盖产品分析
  const invMap = JSON.parse(await eval_('JSON.stringify(STATE.invMap)') || '{}');
  const coveredSkus = new Set(cards.map(c => c.sku));
  const coveredAsins = new Set(cards.map(c => c.asin).filter(Boolean));
  const alerts = [];

  for (const [sku, inv] of Object.entries(invMap)) {
    if (coveredSkus.has(sku) || (inv.asin && coveredAsins.has(inv.asin))) continue;

    // 有库存（fulFillable+reserved>0）有销量但没广告 → 广告可能被关了
    const hasStock = (inv.fulFillable || 0) + (inv.reserved || 0) > 0;
    if (inv.unitsSold_30d > 10 && hasStock) {
      alerts.push({ sku, asin: inv.asin, fulFillable: inv.fulFillable, reserved: inv.reserved, unitsSold_30d: inv.unitsSold_30d, reason: '⚠️ 有销量有库存但无广告活动，广告可能被关闭' });
    }
  }

  // 有广告但本次未调整的产品（有花费但无动作）
  const skippedWithSpend = cards.filter(c => c.adStats['30d'].spend > 0 && !plan.find(p => p.sku === c.sku));
  for (const c of skippedWithSpend.slice(0, 20)) {
    const inv = invMap[c.sku] || {};
    let reason = '';
    if (inv.invDays === 0 && c.adStats['30d'].orders === 0) reason = '无库存且无广告订单';
    else if (c.adStats['30d'].spend <= 0) reason = '30天无花费';
    else if (c.adStats['30d'].orders === 0 && c.adStats['30d'].spend <= 3) reason = `0转化但花费$${c.adStats['30d'].spend.toFixed(2)}<阈值$3，暂不调整`;
    else reason = 'ACOS在目标区间内，无需调整';
    log(`  跳过 ${c.sku}: ${reason} (花费$${c.adStats['30d'].spend.toFixed(2)} 订单${c.adStats['30d'].orders} invDays=${inv.invDays||0})`);
  }

  if (alerts.length > 0) {
    log('=== ⚠️ 需人工关注 ===');
    for (const a of alerts) {
      log(`  ${a.sku} (${a.asin}): ${a.reason} | 近30天销量${a.unitsSold_30d} FBA可售${a.fulFillable} 在途${a.reserved}`);
    }
  }
