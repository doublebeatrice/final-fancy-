...
const OUT = path.join(ROOT, 'data', 'actions', 'rho0122_b2b_kw_group_2026-06-08.json');
...
const PLAN = {
  ...
  siteId: 4,
  mode: 'keywordTarget',
  campaignName: 'b2b kw broad_will you be my bridesmaid labels_rho0122',
  groupName: 'b2b kw broad_will you be my bridesmaid labels_rho0122',
  coreTerm: 'will you be my bridesmaid labels',
  matchType: 'BROAD',
  ...
  defaultBid: 0.37,
  keywords: [
    'will you be my bridesmaid labels',
    'bridesmaid vinyl decal',
    'bridesmaid cards',
  ],
};
...
async function main() {
  ...
  try {
    ...
    if (!createOk || !createMeta.campaignId || !createMeta.adGroupId) {
      const out = { exportedAt: new Date().toISOString(), startedAt, businessDate: '2026-06-08', plan: PLAN, dryRun: built, execution: { ok: false, response, createMeta } };
    ...
    const out = {
      ...
      diagnosis: 'RHO0122 has B2B auto but no B2B keyword lane. Create a narrow SP B2B broad keyword lane from same-SKU proven buyer-facing terms.',
      bidEvidence: {
        sku7dCpc: 0.2395,
        b2bAuto30dConvertingBucketCpc: 0.336666,
        b2bAutoConvertingBucketBid: 0.37,
        source: 'live RHO0122 SKU summary and B2B auto/keyword readback on 2026-06-08',
      },
      keywordEvidence: [
        { keywordText: 'will you be my bridesmaid labels', source: 'same-SKU kw group 30d', orders: 8, acos: 0.142248, cpc: 0.225735 },
        { keywordText: 'bridesmaid vinyl decal', source: 'same-SKU kw group 30d', orders: 2, acos: 0.090058, cpc: 0.199285 },
        { keywordText: 'bridesmaid cards', source: 'same-SKU kw group 30d', orders: 1, acos: 0.28476, cpc: 0.232727 },
      ],
      plan: PLAN,
      dryRun: built,
      execution: {
    ...
    console.log(JSON.stringify({
      ...
      landed: landedRows.map(row => ({ keywordText: row.keywordText, matchType: row.matchType, bid: row.bid, state: row.state, campaignState: row.campaignState, groupState: row.groupState })),
      missingAfter,
    }, null, 2));
