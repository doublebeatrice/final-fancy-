...
const OUT = path.join(ROOT, 'data', 'actions', 'qaa2627_b2b_auto_2026-06-12.json');
...
const PLAN = {
  ...
  siteId: 4,
  mode: 'auto',
  campaignName: 'ai_auto_b2b fathers day christian necklaces_qaa2627',
  groupName: 'ai_auto_b2b fathers day christian necklaces_qaa2627',
  coreTerm: 'b2b fathers day christian necklaces',
...
async function verifyAutoRows(ws, createMeta) {
  ...
  const bidMatches = last.rows.every(row => Number(row.bid) === PLAN.defaultBid);
  return {
    attempts,
...
async function execute() {
  ...
  const out = {
    ...
    dryRun: DRY_RUN,
    evidenceBoundary: 'live ad backend via shared Chrome debug session; sellerinventory productAnalysis live read; GBrain checked for QAA2627/Fathers Day/B2B/auto history.',
    diagnosis: 'Operator requested Fathers Day scale for QAA2627 by adding one Amazon Business SP auto group at bid 0.50. Live reads show QAA2627 is normal-sale and ELIGIBLE, with no existing B2B/business-named SP auto lane. Existing active layers are keyword, ordinary auto, and ASIN target under Mothers Day bulk naming.',
    bidEvidence: {
      sku7dCpc: 0.18,
      sku30dCpc: 0.2791,
      productLineCpc: 0.242179,
      currentAuto7dCpc: 0.14,
      currentAuto30dCpc: 0.12,
      lifetimeCpc: 0.2543,
      selectedB2bAutoBid: PLAN.defaultBid,
      operatorSpecifiedBid: 0.5,
      sourceFiles: [
        'data/snapshots/ad_sku_summary_QAA2627_7d_2026-06-12.json',
        'data/snapshots/ad_sku_summary_QAA2627_30d_2026-06-12.json',
        'data/snapshots/sku_ad_product_QAA2627_7d_pre_b2b_2026-06-12.json',
        'data/snapshots/sku_ad_product_QAA2627_30d_pre_b2b_2026-06-12.json',
        'data/snapshots/product_analysis_query2_QAA2627_2026-06-12.json',
      ],
    },
    productEvidence: {
      asin: PLAN.asin,
      ...
      localInv: 0,
      sales30: 12,
      currentTitle: 'Tondiamo 12 Pcs Christian Mothers Day Gifts Necklaces for Women Inspirational Scripture Pendant Set with Words of Love & Encouragement Religious Faith Gifts for Friends Sisters Coworker Church Member',
    },
    plan: PLAN,
    built,
    execution: null,
