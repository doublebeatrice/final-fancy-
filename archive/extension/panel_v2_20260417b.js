...
async function fetchAllData() {
  ...
  try {
    ...
    // 5. 构建产品画像
    STATE.productCards = buildProductCards(STATE.kwRows, STATE.autoRows, STATE.invMap, {}, STATE.targetRows);
...
async function fetchAllInventory() {
  ...
  if (!capture) throw new Error('30秒内未捕获到库存请求，请手动点击页面查询按钮后重试');
  ...
  if (total > rows.length) {
    ...
    const baseUrl = capture.url
      ? capture.url.replace(/[?&]page=\d+/g, '').replace(/[?&]pageNum=\d+/g, '')
      : null;
    const baseBody = capture.body
      ? String(capture.body).replace(/[&]?pageNum=\d+/g, '').replace(/[&]?page=\d+/g, '')
      : null;
            ...
            func: async (pageUrl, bodyStr, extraHdrs, pageNum) => {
              try {
                ...
                if (pageUrl && bodyStr) {
                  ...
                  res = await fetch(`${pageUrl}${sep}page=${pageNum}&limit=50`, {
                  ...
                  res = await fetch('/pm/formal/list', {
                    method: 'POST', credentials: 'include',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json, text/javascript, */*; q=0.01', 'X-Requested-With': 'XMLHttpRequest', ...extraHdrs },
                    body: `${bodyStr}&pageNum=${pageNum}&limit=50`,
                  });
                } else return null;
...
async function fetchAllKeywords() {
      ...
      chrome.scripting.executeScript(
        { target: { tabId: tab.id }, world: 'MAIN',
          func: async (url, baseBody, hdrs, pageField, key) => {
            ...
            for (let p = 2; p <= 500; p++) {
              try {
                const body = JSON.stringify({ ...baseBody, [pageField]: p, limit: 100 });
            ...
            window[key] = all;
            return all.length;
          }, args: [capture.url, sliceBodyObj, capture.headers || {}, pageField, sliceKey] },
  ...
  if (Array.isArray(bodyObj.timeRange) && bodyObj.timeRange.length === 2) {
    const [tStart, tEnd] = bodyObj.timeRange;
    const SLICE_MS = 7 * 24 * 60 * 60 * 1000;
    const sliceCount = Math.ceil((tEnd - tStart) / SLICE_MS);
    log(`关键词时间切片：${sliceCount} 段（并行）`);
    const sliceBodies = Array.from({ length: sliceCount }, (_, s) => ({
      ...bodyObj,
      timeRange: [tStart + s * SLICE_MS, Math.min(tStart + (s+1) * SLICE_MS, tEnd)],
      [pageField]: 1, limit: 100,
    }));
    await Promise.all(sliceBodies.map((b, i) => fetchSliceInPage(b, `__kwSlice${i}`)));
    log(`关键词全部切片完成：共 ${rows.length} 条`);
  } else {
    await fetchSliceInPage(bodyObj, '__kwSlice0');
  }
...
async function fetchAllAutoTargets(kwCapture) {
  ...
  const baseBody = { ...kwBody, property: '2', tableName: 'product_target',
  ...
  if (Array.isArray(baseBody.timeRange) && baseBody.timeRange.length === 2) {
    const [tStart, tEnd] = baseBody.timeRange;
    const SLICE_MS = 7 * 24 * 60 * 60 * 1000;
    const sliceCount = Math.ceil((tEnd - tStart) / SLICE_MS);
    const sliceBodies = Array.from({ length: sliceCount }, (_, s) => ({
      ...baseBody, timeRange: [tStart + s * SLICE_MS, Math.min(tStart + (s+1) * SLICE_MS, tEnd)],
    }));
    const results = await Promise.all(sliceBodies.map((b, i) => fetchSliceInPage(b, `__atSlice${i}`)));
    for (const res of results) {
      if (res.debug) log(`自动投放诊断：${res.debug} rows=${res.rowCount}`, 'warn');
    ...
    if (res.err) log(`自动投放错误：${res.err}`, 'error');
    for (const row of res.rows) { const id = String(row.id || row.keywordId || ''); if (id && !seen.has(id)) { seen.add(id); rows.push(row); } }
  }
...
async function fetchAllTargeting(kwCapture) {
  ...
  const baseBody = { ...kwBody, property: '3', tableName: 'product_manual_target',
                     filterArray: { campaignState: kwBody.filterArray?.campaignState || '4' } };
  const pageField = 'page' in kwBody ? 'page' : 'pageNum' in kwBody ? 'pageNum' : 'page';
  ...
  if (Array.isArray(baseBody.timeRange) && baseBody.timeRange.length === 2) {
    ...
    const results = await Promise.all(sliceBodies.map((b, i) => fetchSliceInPage(b, `__tgSlice${i}`)));
    for (const res of results)
...
function buildInvMap(rows) {
  ...
  for (const r of rows) {
    ...
    // 只处理正常销售和保留页面的产品
    if (r.sale_status && r.sale_status !== '正常销售' && r.sale_status !== '保留页面') continue;
...
function buildProductCards(kwRows, autoRows, invMap, listingMap, targetRows = []) {
  ...
  function getOrCreateCampaign(skuObj, campaignId, campaignName) {
    if (!skuObj.campaigns[campaignId]) {
      skuObj.campaigns[campaignId] = { campaignId, name: campaignName || '', keywords: [], autoTargets: [] };
    }
  ...
  for (const r of kwRows) {
    ...
    // 从 campaignName / groupName 里匹配库存 SKU
    if (!sku) {
      const campUp = (r.campaignName || r.groupName || '').toUpperCase();
      const hit = invSkus.find(s => campUp.includes(s.up));
      if (hit) { sku = hit.raw; if (!asin) asin = invMap[hit.raw]?.asin || ''; }
    }
    ...
    camp.keywords.push({
      ...
      onCooldown,
      stats7d: {
        spend: parseFloat(r.spend7 || r['7天花费'] || 0),
        orders: parseFloat(r.orders7 || r['7天订单'] || 0),
        acos: parseFloat(r.acos7 || r['7天ACOS'] || 0),
        clicks: parseFloat(r.clicks7 || r['7天点击'] || 0),
        impressions: parseFloat(r.impressions7 || r['7天曝光'] || 0),
      },
      stats30d: {
        spend: parseFloat(r.Spend || r.spend30 || r['花费'] || r['30天花费'] || 0),
        orders: parseFloat(r.Orders || r.orders30 || r['广告订单量'] || 0),
        acos: parseFloat(r.ACOS || r.Acos || r.acos30 || r['30天ACOS'] || 0),
        clicks: parseFloat(r.Clicks || r.clicks30 || r['点击量'] || 0),
        impressions: parseFloat(r.impressions30 || r['曝光量'] || 0),
      },
    });
  }
  ...
  for (const r of autoRows) {
    ...
    camp.autoTargets.push({
      ...
      onCooldown,
      stats7d: {
        spend: parseFloat(r.spend7 || 0),
        orders: parseFloat(r.orders7 || 0),
        acos: parseFloat(r.acos7 || 0),
        clicks: parseFloat(r.clicks7 || 0),
        impressions: parseFloat(r.impressions7 || 0),
      },
      stats30d: {
        spend: parseFloat(r.Spend || r.spend30 || r['花费'] || 0),
        orders: parseFloat(r.Orders || r.orders30 || r['广告订单量'] || 0),
        acos: parseFloat(r.ACOS || r.Acos || r.acos30 || r['30天ACOS'] || 0),
        clicks: parseFloat(r.Clicks || r.clicks30 || r['点击量'] || 0),
        impressions: parseFloat(r.impressions30 || 0),
      },
    });
  }
  ...
  for (const r of targetRows) {
    ...
    camp.autoTargets.push({
      ...
      stats7d: { spend: 0, orders: 0, acos: 0, clicks: 0, impressions: 0 },
      stats30d: {
        spend: parseFloat(r.Spend || r.spend30 || 0),
        orders: parseFloat(r.Orders || r.orders30 || 0),
        acos: parseFloat(r.ACOS || r.Acos || 0),
        clicks: parseFloat(r.Clicks || r.clicks30 || 0),
        impressions: 0,
      },
    });
  }

  // 只保留 invMap 里有的产品（正常销售/保留页面），按 SKU 或 ASIN 匹配
  const invByAsin = {};
  for (const v of Object.values(invMap)) { if (v.asin) invByAsin[v.asin] = v; }
  return Object.values(skuMap).filter(card => invMap[card.sku] || (card.asin && invByAsin[card.asin])).map(card => {
    const campaigns = Object.values(card.campaigns);
    // 聚合广告统计
    const adStats = { '7d': { spend:0,orders:0,clicks:0,impressions:0 }, '30d': { spend:0,orders:0,clicks:0,impressions:0 } };
    for (const camp of campaigns) {
      for (const kw of camp.keywords) {
        for (const [dim, s] of [['7d', kw.stats7d], ['30d', kw.stats30d]]) {
      ...
      for (const at of camp.autoTargets) {
        for (const [dim, s] of [['7d', at.stats7d], ['30d', at.stats30d]]) {
          adStats[dim].spend += s.spend; adStats[dim].orders += s.orders;
          adStats[dim].clicks += s.clicks; adStats[dim].impressions += s.impressions;
        }
      }
    }
    for (const dim of ['7d', '30d']) {
      const s = adStats[dim];
...
function buildClaudePrompt(cards) {
  ...
  const slim = cards.map(c => ({
    ...
    keywords: c.campaigns.flatMap(camp => camp.keywords.map(kw => ({
      ...
      stats7d: kw.stats7d, stats30d: kw.stats30d,
    }))),
    autoTargets: c.campaigns.flatMap(camp => camp.autoTargets.map(at => ({
      ...
      stats7d: at.stats7d, stats30d: at.stats30d,
    }))),
  }));
...
3. 给出广告策略方向
4. 给出具体调整动作（哪些关键词/自动投放，建议竞价是多少，原因是什么）
...
function renderCards(results, batch) {
  ...
  for (const r of results) {
    ...
    cardEl.dataset.sku = r.sku;

    const stageTag = r.stage ? `<span class="tag tag-stage-${r.stage}">${r.stage}</span>` : '';
    const healthTag = r.health ? `<span class="tag tag-health-${r.health}">${r.health}</span>` : '';
    const alertHtml = r.listingAlert ? `<span class="listing-alert">⚠ ${r.listingAlert}</span>` : '';
    const asinLink = r.asin ? `<span class="card-asin">${r.asin}</span>` : '';
    const actCount = (r.actions || []).length;

    cardEl.innerHTML = `
      <div class="card-header" onclick="toggleCard('${r.sku}')">
        <span class="card-sku">${r.sku || r.asin}</span>
        ${asinLink}
        ${stageTag}${healthTag}${alertHtml}
        <span class="card-strategy">${r.strategy || ''}</span>
        <span class="card-toggle" id="toggle-${r.sku}">${actCount} 个动作 ▼</span>
      </div>
      <div class="card-body" id="body-${r.sku}" style="display:none">
...
function renderActionsTable(actions, result) {
  ...
  for (const a of actions) {
    const isUp   = a.suggestedBid > a.currentBid;
    const isDown = a.suggestedBid < a.currentBid;
    const cls    = isUp ? 'bid-up' : isDown ? 'bid-down' : 'bid-same';
    const arrow  = isUp ? '↑' : isDown ? '↓' : '—';
    const onCooldown = checkCooldown(a.id, result);
    const defaultChecked = !onCooldown && result.health !== 'Listing异常';
    const cooldownBadge = onCooldown ? '<span class="cooldown-badge">冷却中</span>' : '';
    rows += `
      <tr>
        <td><input type="checkbox" class="action-check" data-id="${a.id}" data-type="${a.entityType}" data-bid="${a.suggestedBid}" data-sku="${result.sku}" ${defaultChecked ? 'checked' : ''}></td>
        <td>${a.entityType === 'keyword' ? '关键词' : '自动投放'}</td>
        <td style="font-size:10px;color:#8b949e">${a.id}</td>
        <td>$${a.currentBid?.toFixed(2) || '-'}</td>
        <td class="bid-change ${cls}">$${a.suggestedBid?.toFixed(2) || '-'} ${arrow}</td>
        <td>${cooldownBadge}</td>
        <td class="reason-text">${a.reason || ''}</td>
      </tr>`;
  }
  return `
    <table class="actions-table">
      <thead><tr><th></th><th>类型</th><th>ID</th><th>当前竞价</th><th>建议竞价</th><th></th><th>原因</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}
...
async function executePlan() {
  ...
  // 按类型分组
  const kwItems = [], atItems = [];
  for (const el of checkedEls) {
    const { id, type, bid, sku } = el.dataset;
    const item = { id, bid: parseFloat(bid) };
    if (type === 'keyword') kwItems.push(item);
  ...
  if (kwItems.length) {
    try {
      await execAdWrite('/keyword/batchKeyword', {
        column: 'bid', property: 'keyword', operation: 'bid',
        idArray: kwItems.map(i => i.id),
        targetArray: kwItems.map(i => ({ id: i.id, bid: i.bid })),
        targetNewArray: kwItems.map(i => ({ id: i.id, bid: i.bid })),
      });
      ok += kwItems.length;
      log(`关键词竞价更新成功：${kwItems.length} 条`, 'ok');
    } catch (e) { err += kwItems.length; log(`关键词更新失败：${e.message}`, 'error'); }
  }

  // 执行自动投放竞价更新
  if (atItems.length) {
    try {
      // 从原始数据里查找完整字段（需要 siteId/accountId/campaignId/adGroupId）
      const atRows = [...STATE.autoRows, ...STATE.targetRows];
      const targetArray = atItems.map(i => {
        const raw = atRows.find(r => String(r.targetId || r.id || '') === String(i.id)) || {};
        return {
          siteId: raw.siteId || 4,
      ...
      const accountId = targetArray[0]?.accountId;
      const siteId = targetArray[0]?.siteId || 4;
      await execAdWrite('/advTarget/batchEditAutoTarget', {
        column: 'bid', property: 'autoTarget', operation: 'bid',
        accountId, siteId,
        idArray: atItems.map(i => i.id),
        campaignIdArray: [...new Set(targetArray.map(t => t.campaignId).filter(Boolean))],
        targetArray,
        targetNewArray: targetArray,
      });
      ok += atItems.length;
      log(`自动投放竞价更新成功：${atItems.length} 条`, 'ok');
    } catch (e) { err += atItems.length; log(`自动投放更新失败：${e.message}`, 'error'); }
  }
...
function saveExecutionLog(checkedEls, ok, err) {
  const record = {
    ...
    ok, err,
    actions: [...checkedEls].map(el => ({ id: el.dataset.id, type: el.dataset.type, bid: el.dataset.bid, sku: el.dataset.sku })),
  };
...
async function exportSnapshot() {
  ...
  // 只导出有广告花费的产品，精简字段
  const cards = STATE.productCards.filter(c => c.adStats?.['30d']?.spend > 0);
  const slim = cards.map(c => ({
    sku: c.sku,
    asin: c.asin || null,
    ...
    keywords: c.campaigns.flatMap(camp => camp.keywords.map(kw => ({
      id: kw.id, text: kw.text, matchType: kw.matchType,
      bid: kw.bid, onCooldown: kw.onCooldown, stats30d: kw.stats30d,
    }))).filter(kw => kw.stats30d?.spend > 0),
    autoTargets: c.campaigns.flatMap(camp => camp.autoTargets.map(at => ({
      id: at.id, targetType: at.targetType,
      bid: at.bid, onCooldown: at.onCooldown, stats30d: at.stats30d,
    }))).filter(at => at.stats30d?.spend > 0),
  }));
