// diagnose.js — 诊断 analyzeCard 各层过滤漏斗
const WebSocket = require('ws');
const { PANEL_ID, loadHistory } = require('./adjust_lib');
...
async function run() {
  ...
  let e_0ord_lowSpend = 0;   // orders=0, spend<=0.5
  let e_0ord_cooldown = 0;   // orders=0, spend>0.5, 但冷却
  let e_0ord_fire = 0;       // orders=0, spend>0.5, 触发
  let e_hasOrd_noTarget = 0; // orders>0, target=0
  let e_hasOrd_midAcos = 0;  // orders>0, target>0, ACOS 在可接受区间
  let e_hasOrd_cooldown = 0; // orders>0, ACOS 触发但冷却
  let e_hasOrd_tinyDiff = 0; // bid 变化 < 0.01
  let e_fire = 0;            // 最终触发
    ...
    for (const camp of card.campaigns) {
      ...
      for (const entity of entities) {
        ...
        if (orders === 0 && spend > 3) {
          ...
          e_hasOrd_noTarget++; // acos=0 也归入此类
        }

        if (newBid) {
          if (Math.abs(newBid - entity.bid) <= 0.01) { e_hasOrd_tinyDiff++; newBid = null; }
        }

        if (newBid) {
          e_fire++;
          reasons[reason.split('$')[0].split('%')[0]] = (reasons[reason.split('$')[0].split('%')[0]] || 0) + 1;
        }
      }
    }
  }

  const e_0ord_fire_actual = e_fire; // approximate
  ...
  console.log(`    其中 target=0 (利润率=0): ${p_noTarget}`);
  ...
  console.log(`  0转化 spend<=0.5 (太低):   ${e_0ord_lowSpend}`);
  console.log(`  0转化 spend>0.5 但冷却:    ${e_0ord_cooldown}`);
  console.log(`  有转化 target=0 (跳过):    ${e_hasOrd_noTarget}`);
