...
const READ_ONLY_ACTIONS = new Set(['fetch', 'diagnose', 'inspect', 'report', 'summarize']);
...
function isLowRiskAdAction(action = {}) {
  const actionType = text(action.actionType || action.type).toLowerCase();
  const entityType = text(action.entityType).toLowerCase();
  return LOW_RISK_AD_ACTIONS.has(actionType) && LOW_RISK_AD_ENTITIES.has(entityType);
}
...
function buildReviewTasks({ sourceTaskId = '', action = {}, timeContext = {} } = {}) {
  ...
  const days = [...new Set((reviewPlan.checkAfterDays || []).map(Number).filter(day => Number.isFinite(day) && day > 0))].sort((a, b) => a - b);
  ...
  return days.map(day => normalizeAgentTask({
    ...
    description: text(reviewPlan.rollbackIf || '复查动作后的订单、花费、ACOS 和库存影响。'),
    subject: {
      sku: action.sku,
      asin: action.asin,
