...
- 执行输出里已经写明复查路径、复查日期、成功信号或失败条件，例如 3 天 / 7 天回看、花费阈值、订单阈值、降 bid / 暂停条件。
...
4. 如果本次已经产出单独 follow-up 文件，也要同步写入本池；单独文件负责细节，本池负责每天把它捞出来。
5. 如果仍有风险，保留 `status=open` 或 `status=watching`，不能口头说完就关闭。
6. 只有 `closeConditions` 全部满足，才能改成 `status=closed`，并保留关闭原因。
